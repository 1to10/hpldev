...
@section('page_heading',(isset($categoryByid))?'Update Product Range':'Add Product Range')
        ...
        <div class="row">
            <div class="col-sm-12">
                @component('admin.widgets.panel')
                    @slot('panelTitle', (isset($categoryByid))?'Update Product Range':'Add Product Range')
                    @slot('panelBody')
                          @if(isset($categoryByid))
                            {!! Form::model($categoryByid, [
                          'method' => 'POST',
                          'route' => ['productrange.update', $categoryByid->id]
                      ]) !!}
                        @else
                        ...
                        <div class="form-group">
                            <label for="sel">Status</label>
                                  <select id="cat" name="parent_id" class="form-control">
                                      @foreach($category as $cat)
                                      <option value="{{$cat->id}}" {{(isset($categoryByid->parent_id) && $cat->id==$categoryByid->parent_id)?'selected':''}}>{{$cat->name}}</option>
                                      @endforeach
