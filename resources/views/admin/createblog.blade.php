...
@section('page_heading',(isset($categoryByid))?'Update Blog':'Add Blog')
        ...
        <div class="row">
            <div class="col-sm-12">
                @component('admin.widgets.panel')
                    @slot('panelTitle', (isset($categoryByid))?'Update Blog':'Add Blog')
                    @slot('panelBody')

                        @if(isset($categoryByid))
                            {!! Form::model($categoryByid, [
                          'method' => 'POST',
                          'route' => ['blog.update', $categoryByid->id]
                      ]) !!}
                        @else
                            {!! Form::open(['url' => 'admin/storeBlog']) !!}
                        @endif
